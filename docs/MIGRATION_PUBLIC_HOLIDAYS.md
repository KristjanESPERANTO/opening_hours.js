...
### Phase 1: Integration & Validierung (teilweise umgesetzt)

**Ziel:** date-holidays als Datenquelle für PH anbinden.

- [x] `date-holidays` als versioniertes Submodul einbinden
- [x] Synchronisationsscript: `scripts/sync_holidays_from_date_holidays.js`
  ```bash
  node scripts/sync_holidays_from_date_holidays.js
  ```
- [x] Konverter: date-holidays-Regeln → opening_hours.js-Format
...
- [x] Bestehende YAMLs bleiben als Fallback und für SH erhalten
- [ ] `date-holidays` als Produktions-Dependency verwenden
- [ ] Synchronisation automatisch in den Build integrieren
...
### Phase 2: Schrittweise Aktivierung (offen)
...
### Phase 3: YAML-Bereinigung (offen)
...
### Phase 4: Upstream Contributions (Optional, parallel)

**Ziel:** Unsere YAML-Daten zu date-holidays beitragen (falls besser).

- [ ] Fälle dokumentieren, in denen unsere YAMLs vollständiger sind
...
| Phase 2: Aktivierung | 2-3 Wochen | Phase 1 |
| Phase 3: YAML-Cleanup | 1 Woche | Phase 2 + 3 Monate |
| Phase 4: Contributions | Parallel zu 2-3 | - |
