...
function formatYamlSimpleScalar(value) {
    const str = String(value);
    const isSafePlainScalar = /^[A-Za-z0-9 ._()\-/À-ÖØ-öø-ÿ'’]+$/u.test(str);
    const looksLikeBooleanOrNull = /^(?:y|yes|n|no|true|false|on|off|null|~)$/i.test(str);

    if (isSafePlainScalar && !looksLikeBooleanOrNull) {
        return str;
    }

    return `'${escapeYamlSingleQuote(str)}'`;
}
...
function generateStateYaml(ccLower, config, existingData) {
    ...
    for (const [stateCode, stateName] of Object.entries(config.states)) {
        ...
        const { holidays: rawHolidays, optionalCount: stateOptionalCount } = fetchPublicHolidays(config.cc, stateCode, stateConfig);
    ...
    for (const { stateCode, stateName, rawHolidays } of orderedStateResults) {
        ...
        if (config.includeStateCode) {
            lines.push(`  _state_code: ${formatYamlSimpleScalar(stateCode.toLowerCase())}`);
        }
