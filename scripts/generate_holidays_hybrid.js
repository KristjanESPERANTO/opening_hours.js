...
const COUNTRY_CONFIGS = {
    ...
    au: {
        cc: 'AU',
        states: AU_STATES,
        includeNationalPh: 'intersection',
        includeStateCode: true,
        stateNominatimUrls: AU_STATE_NOMINATIM_URLS,
    },
    be: { cc: 'BE', nameLangs: ['nl', 'fr'] },
    br: { cc: 'BR', statesFromSource: true, includeStateCode: true, includeNationalPh: true, stateAcceptLanguage: 'pt,en' },
    ca: { cc: 'CA' },
    ...
    ch: { cc: 'CH', skip: 'CH has 26 cantons with complex only_states logic' },
...
function parseRule(rule) {
    ...
    // "MM-DD if ..." (without "and") = replacement substitute (only shifted date).
...
function getStatesFromCountryYaml(ccUppercase) {
    ...
    for (const [stateCode, stateData] of Object.entries(countryData.states)) {
        ...
        if (!stateData.name || typeof stateData.name !== 'string') continue;
 ...
 */
function getMultiLanguageHolidayName(rule, dayData, nameLangs, canonicalNameKey) {
    ...
    if (dayData && dayData.name && typeof dayData.name === 'object') {
        Object.assign(namesByLang, dayData.name);
    }
...
function fetchPublicHolidays(cc, stateCode, options = {}) {
    ...
    if (!yamlData || !yamlData.holidays || !yamlData.holidays[cc]) {
        console.warn(`  ⚠️  No YAML found for ${cc}, skipping`);
        return [];
    }

    const countryData = yamlData.holidays[cc];
    if (!countryData.days) return [];
    ...
    function addHoliday(rule, dayData, countryLangs) {
        if (!isIncludedDayType(dayData)) return;
        if (!isRuleRelevantByRuleText(rule, today)) return;
        if (!isRuleActiveInYear(dayData, today)) return;
        ...
        // Keep explicit names authoritative only if they include a preferred local language.
        let finalName = (hasExplicitName && hasPreferredLanguage)
            ? yamlName
            : resolveHolidayName(rule, yamlName, countryLangs, canonicalNameKey);

        const multiLanguageName = getMultiLanguageHolidayName(rule, dayData, options.nameLangs, canonicalNameKey);
        if (multiLanguageName) {
            finalName = multiLanguageName;
        }

        holidays.push({ name: finalName, rule });
    ...
    for (const [rule, dayData] of Object.entries(countryData.days)) {
        ...
        // Skip rules that start with "substitutes ...".
        if (/^substitutes?\b/i.test(rule)) continue;
        addHoliday(rule, dayData, countryLangs);
    }
 ...
 */
function buildPhLines(rawHolidays, indent, countryCode, stateCode) {
    ...
    for (const { name, rule } of skipped) {
        ...
        console.warn(`  ⚠️  Skipped unsupported rule [${loc}] "${name}": ${rule}`);
    }
...
function generateSimpleYaml(ccLower, config, existingData) {
    ...
    const nominatimUrl = getNominatimUrl(existingData);
    const sh = extractSH(existingData);

    const lines = [...GENERATED_YAML_HEADER];

    if (nominatimUrl) {
        lines.push(`_nominatim_url: '${nominatimUrl}'`);
        lines.push('');
    }

    lines.push(...buildPhLines(rawHolidays, '', ccLower, null));

    if (sh) {
        lines.push('');
        lines.push('SH:');
        // Re-serialize SH entries as inline YAML (best effort: stringify as JSON-like)
...
function generateStateYaml(ccLower, config, existingData) {
    ...
    const countryNominatimUrl = getNominatimUrl(existingData);
    if (countryNominatimUrl) {
        lines.push(`_nominatim_url: '${countryNominatimUrl}'`);
        lines.push('');
    }

    // Top-level SH (if any)
    const countrySH = extractSH(existingData);
    if (countrySH) {
        lines.push('SH:');
        for (const entry of countrySH) {
            lines.push(`  - ${JSON.stringify(entry)}`);
        }
        lines.push('');
    }
    ...
    if (config.includeNationalPh === true) {
        ...
        lines.push(...buildPhLines(nationalRawHolidays, '', ccLower, null));
        lines.push('');
    }

    if (config.includeNationalPh === 'intersection') {
        let common = null;
        // Per-state full-def maps (with rules) to detect rule consensus later.
        const stateFullMaps = [];

        for (const stateResult of stateResults) {
            const { holidays } = convertHolidays(stateResult.rawHolidays);
            const stateMap = new Map();
            const stateFullMap = new Map();
            for (const h of holidays) {
                // Exclude shift_rule and substitute_rule from the intersection key:
                // different states may have different shift/substitute rules for the
                // same holiday (e.g. AU New Year's Day).
                const baseDefForKey = { ...h.def };
                delete baseDefForKey.shift_rule;
                delete baseDefForKey.substitute_rule;
                const key = `${h.name}::${JSON.stringify(baseDefForKey)}`;
                stateMap.set(key, { name: h.name, def: baseDefForKey });
                // Keep the full def (last entry wins when a state has multiple entries
                // for the same base key, e.g. time-limited variants).
                stateFullMap.set(key, h.def);
            }
            stateFullMaps.push(stateFullMap);
        ...
        const commonHolidays = common
            ? Array.from(common.entries()).map(([key, { name, def: baseDef }]) => {
                // If all states agree on shift_rule / substitute_rule, propagate it
                // to the national entry; otherwise omit it (states differ).
                const finalDef = { ...baseDef };
                for (const field of ['shift_rule', 'substitute_rule']) {
                    const first = stateFullMaps[0]?.get(key)?.[field];
                    if (first !== undefined && stateFullMaps.every(m => m.get(key)?.[field] === first)) {
                        finalDef[field] = first;
                    }
                }
                return { name, def: finalDef };
            }).sort(compareGeneratedHolidays)
            : [];

        lines.push('PH:');
        for (const { name, def } of commonHolidays) {
    ...
    for (const { stateCode, stateName, rawHolidays } of stateResults) {
        ...
        lines.push(`'${escapeYamlSingleQuote(stateName)}':`);
        ...
        lines.push(...buildPhLines(rawHolidays, '  ', ccLower, stateCode));
